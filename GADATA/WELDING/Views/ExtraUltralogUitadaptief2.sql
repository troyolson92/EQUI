...
AS
SELECT        WELDING.Qcontrol.spot, MAX(dbo.UltralogInspections.InspectionTime) AS Expr1
FROM            dbo.UltralogInspections INNER JOIN
                         dbo.Spot ON dbo.UltralogInspections.SpotID = dbo.Spot.ID INNER JOIN
                         WELDING.Qcontrol ON dbo.Spot.Number = WELDING.Qcontrol.spot
WHERE        (WELDING.Qcontrol.[uit adaptief] = 1)
GROUP BY WELDING.Qcontrol.spot
GO
