...
-- =============================================
CREATE PROCEDURE [WELDING].[WeldingTimers] 
...
----Alles uit de spottabbellen BOS6000----  OK werkt!


INSERT INTO WELDING.spot (Timer,Spot,Program)
SELECT     Timername,SpotName,WeldProgNo 
FROM
 (
...
FROM [19.148.192.33\BOS_SQLSERV_2005].[BOS_6000_DB].[dbo].[ExtSpotTable_V]
                         ...
                         WELDING.spot ON spottable.SpotName = WELDING.spot.Spot
WHERE        (Welding.Spot.spot IS NULL)

---Keep spottable up to date with deleted spots---  Nog NOK => te bekijken

DELETE 
FROM WELDING.spot 
WHERE welding.spot.spot NOT IN (

SELECT DISTINCT SpotName 
FROM [10.249.203.138\BOS_SQLSERV_2005].[BOS_6000_DB].[dbo].[ExtSpotTable_V]
UNION
SELECT DISTINCT SpotName COLLATE DATABASE_DEFAULT
FROM [10.249.222.197\BOS_SQLSERV_2005].[BOS_6000_DB].[dbo].[ExtSpotTable_V]
UNION
SELECT DISTINCT SpotName COLLATE DATABASE_DEFAULT
FROM [10.249.222.198\BOS_SQLSERV_2005].[BOS_6000_DB].[dbo].[ExtSpotTable_V]
...
-------insert double spots from GA into Welding.doubleSpot table------  NOG niet OK => moet nog deleten in voorgaande query-----


---DELETE FIRST---
DELETE FROM WELDING.doublespots
---INSERT NEW DATA----
INSERT INTO WELDING.doublespots

SELECT         Welding.spot.Spot, Welding.spot.Program, Welding.spot.Timer
FROM            Welding.spot 
WHERE        (Welding.spot.Spot IN
                             (SELECT        SpotTable_1.Spot
                               FROM            Welding.spot AS SpotTable_1 
                                             
                               WHERE        (Welding.spot.Timer <> N'NN99010WT02') AND (Welding.spot.Timer <> N'NN99070WT01') AND (Welding.spot.Timer NOT LIKE N'%WN%')
                               GROUP BY SpotTable_1.Spot
                               HAVING         (COUNT(SpotTable_1.Spot) > 1)))
ORDER BY Welding.spot.Spot
...
---- insert parameter settings from NPT's into welding.spottable------
----UIT adaptief----
--NPT01---
----update regulation---
UPDATE WELDING.spot 
