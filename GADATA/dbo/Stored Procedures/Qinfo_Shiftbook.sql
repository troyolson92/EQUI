...
END 
	...
	SELECT 
		...
		,ISNULL(b.refId,0) as 'Breakdownreference'
	    ,ISNULL(sb.id,0) as 'ShiftbookID'
		,@Redirect as 'Redirect'
		,'*****************************************' 
		,ISNULL(ISNULL(sb.userDescription,'BREAKDOWN: ' + b.logtext),'') as 'Omschrijving'
	    ,ISNULL(b.[Downtime(s)]/60,0) as 'Downtime'
	...
	left join
	(
	select * from GADATA.C3G.Breakdown as c3gb where  c3gb.refId = @breakdownid
	UNION
	select * from GADATA.C4G.breakdown as c4gb where  c4gb.refId = @breakdownid
	) as B on b.controller_name = @Robot
	WHERE
