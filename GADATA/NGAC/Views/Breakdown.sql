...
AND
A.controller_id = rtj.c_controller_id --join the right 'data controller id'
AND 
--USE ASSET NUM to JOIN see below.
A.ASSETNUM  LIKE  'UR%'
AND 
A.controller_ToolID = 1 -- WHY ???

--join controller
LEFT JOIN NGAC.c_controller as c on c.id = rtj.c_controller_id
