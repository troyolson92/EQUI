...
CREATE VIEW [NGAC].[devicePropertyLog]
AS
select 
...
, 'Prop: <' +  c_D.name +  '> Value: <' 
+ ISNULL(CAST((lag(rt.value) OVER (PARTITION BY rt.c_controller_id, rt.c_device_info_id ORDER BY rt._timestamp desc)) as varchar(max)),'N/A')
+ '> -> <' + CAST(rt.value as varchar(max)) + '>'AS 'FullLogtext'
, NULL     AS 'Response'
, NULL     AS 'Downtime'
, ''  AS 'Classification'
, ''	 AS 'Subgroup'
...
FROM [GADATA].[NGAC].[rt_device_info] as rt
left join GADATA.NGAC.c_device_info as c_d on c_d.id = rt.c_device_info_id
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'NGAC', @level1type = N'VIEW', @level1name = N'devicePropertyLog';
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'NGAC', @level1type = N'VIEW', @level1name = N'devicePropertyLog';
