...
AND 
--USE ASSET NUM to JOIN see below.
A.ASSETNUM + '%' LIKE  ISNULL(RTRIM(cc.Classification),'UR') + '%'   --join only the asset with the right classification. (if not classified data goes to robot)
-- SDEBEUL A LOT OFF CLASSIFCATION ARE NOT FILLED IN MX7 
